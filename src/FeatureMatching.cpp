...
FeatureMatching::FeatureMatching( const char* voc_dataset, Config::Ptr config) {
    // 设置bow阈值条件
    mVoc_ = std::make_shared<DBoW3::Vocabulary>();
    mVoc_->load(voc_dataset);
    mTh_ = config->GetParam<int>("Threshold");
    mNNRatio_ = config->GetParam<float>("NNRatio");
}
...
// 利用暴力匹配法进行特征匹配
bool FeatureMatching::MatchByBruteForce( cv::Mat& desp1, cv::Mat& desp2, std::vector<cv::DMatch>& matches ) {
...
// 利用投影法进行特征匹配
bool FeatureMatching::MatchByProject( std::vector<cv::Point3d>& vp3d1, cv::Mat& desp1, 
...
// 计算两个描述子间的匹配分数
int FeatureMatching::ComputeMatchingScore( cv::Mat& desp1, cv::Mat& desp2 ) {
